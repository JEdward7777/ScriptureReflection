...
with tabs[0]:
    ...
    if selected_translation:
        ...
        chapter = st.number_input("Select Chapter", min_value=1, value=1)
        verse = st.number_input("Select Verse", min_value=1, value=1)
        ...
        with col1:
            if st.button("Previous"):
                if verse > 1:
                    ...
                    chapter -= 1
                    verse = max(split_ref(item['vref'])[2] for item in translation_data if split_ref(item['vref'])[0] == book and split_ref(item['vref'])[1] == chapter)
        with col2:
            if st.button("Next"):
                max_verse = max(split_ref(item['vref'])[2] for item in translation_data if split_ref(item['vref'])[0] == book and split_ref(item['vref'])[1] == chapter)
                if verse < max_verse:
                    verse += 1
                else:
